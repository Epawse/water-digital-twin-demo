...
class Controller {
  ...
  init(BaseMapConfig: any, MapImageryList: any) {
    ...
    let imageryProviderConfig = new Cesium.SingleTileImageryProvider({
      // url: process.env.VUE_APP_API_ASSETS + 'Cesium-1.82-epawse/background.png',
      // url: 'https://mapv-data.oss-cn-hangzhou.aliyuncs.com/Cesium-1.82-epawse/background.png',
      url: 'Cesium-1.82-epawse/background.png', // Use local public path
    })
  ...
  setConfigMapList(viewer: any, MapImageryList: any) {
    ...
    MapImageryList.some((elem: any, index: number) => {
      ...
      // 注意：已使用 Globe-level Filter 替代，这里不再调用以避免双重滤镜
      // if (elem.invertswitch) {
